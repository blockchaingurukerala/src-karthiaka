...
//Get Metamask Account
  ...
  render: async () => {
    ...
    var count= await App.medicine.orderStatusCountEndUser();
    ...
    if(App.distributordisplay==1){
      ...
      for (var i = 1; i <= count; i++) {
        var medicine=await App.medicine.orderstatusesofenderusers(i);
        var accountaddrees=medicine[4];
        var id=medicine[1];
          if(id!=0){
            ...
            var qty=medicine[2];
            var distributer=medicine[3];
            
                 
                    var medicinedetails=await App.medicine.medicines(parseInt(id));       
                    
                    var medname=medicinedetails[1];      
                    var user=await App.medicine.users(medicinedetails[2]);
                    var manfact=user.name; 
                    var user1=await App.medicine.users(distributer);
                    var distributername=user1.name;

            if(accountaddrees.toUpperCase().localeCompare(App.account.toUpperCase())==0){
              var str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td></tr>";
      ...
      for (var i = 1; i <= orderStatusCountEndUser; i++) {      
        ...
        var accountaddrees=OrderStatusEndUser[4];         
        ...
        if(accountaddrees.toUpperCase().localeCompare(App.account.toUpperCase())==0){
          ...
          var str="";
          if(status=="0"){
            str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td><tr>"                 
          }
          if(status=="1"){
            //purchased by END USER Need to accept
            str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td><td>Purchased.Waiting for Accept</td><tr>"  
           
          }
          if(status=="2"){
            //Accepted the order Need to ship
            str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td><td>Accepted.Waiting for Shipping</td><tr>"   
           
          }
          if(status=="3"){
            //Product Shipped MArk as wating for Delivery confirmation
            str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td><td>Shiped.Waiting For Delivered</td><td><button class='btn btn-info' onclick='App.markSatusAsDelivered(`"+i+"`)'>Mark as Delivered</button></td><tr>"   
           
          }
          if(status=="4"){
            //Product Shipped MArk as wating for Delivery confirmation
            str = "<tr><td>" + medname +"</td><td>"+manfact+"</td><td>"+distributername+"</td><td>"+qty+"</td><td>Product Delivered</td><tr>"  
  ...
  markSatusAsDelivered :async (id)=>{
    await App.medicine.updateOrderStatusEndUser(parseInt(id),"4",{from:App.account});
    await App.render();
  },
